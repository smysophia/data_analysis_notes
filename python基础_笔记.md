...
1. 列表推导式
   ...
   * 使用()生成迭代器generator, 例如 `multiples = (i for i in range(30) if i%3 == 0)`
2. 字典推导式
...
* 从字符串的第一个字符开始匹配，如果未匹配到返回None，匹配到则返回一个<对象>
* r.group() 返回找到的结果
* r.span() 返回一个元组表示匹配位置（开始，结束）   

`re.search(pattern, string)`   
* 是搜索整个字符串然后第一个匹配到指定的字符则返回值，未匹配到则返回None.需要通过group()来获取匹配到的值
